...
import { Backup as BackupService } from '../../../src/application/service/backup.service';
...
describe('Class FMP Service', () => {
  it('etfHolder - Happy Path', async () => {
    const fmp: FMPService = new FMPService(new ApiRestUtil('url'));
  ...
  it('historicalPriceFull - Happy Path', async () => {
    ...
    const filePath1 = `${ConstantsUtil.BACKUP_BASE_PATH()}fmp-historical-price-full-VOO${
          ...
          historical: [
            {
              adjClose: 385.869995,
              change: -0.245,
              changeOverTime: -0.000634526,
              changePercent: -0.0634526,
              close: 385.87,
              date: '2023-05-01',
              high: 386.33,
              label: 'May 01, 23',
    ...
    const filePath2 = `${ConstantsUtil.BACKUP_BASE_PATH()}fmp-historical-price-full-VOO-VIG${
      BackupService.fileExtension
    }`;
    await fs.promises.writeFile(
      filePath2,
    ...
    let historical = await fmp.historicalPriceFull('VOO,VIG');
    ...
    fs.readdir(ConstantsUtil.BACKUP_BASE_PATH(), (err, files) => {
      files.forEach((file: string) => {
        ...
        if (
          file.startsWith(prex) === true &&
          file
            .substring(prex.length, file.length - 5)
            .split(',')
            .filter((s) => {
              return ['VIG', 'VOO'].includes(s) === true;
            }).length > 0
        ) {
          try {
            fs.unlinkSync(`${ConstantsUtil.BACKUP_BASE_PATH()}\\${file}`);
          } catch (error) {
    ...
    historical = await fmp.historicalPriceFull('VOO,VIG');
    expect(Array.isArray(historical)).toBeTruthy();
    expect(historical.length).toBe(2);
    expect(historical[0].symbol).toBe('VIG');
    expect(historical[1].symbol).toBe('VOO');
  ...
  it('saveDate - invalid backup path', async () => {
    const fmp: FMPService = new FMPService(
      undefined,
      ConfigurationUtil.FmpApiKey()
    );
    expect(
      await fmp.saveDate(
        ...
        `xxx\\test${BackupService.fileExtension}`
      )
    ).not.toEqual({});
  });
  it('saveDate - old data', async () => {
    const fmp: FMPService = new FMPService(
      undefined,
      ConfigurationUtil.FmpApiKey()
    );
    await fmp.saveDate(
      'https://fintual.cl/api/banks',
      `backup\\test-1${BackupService.fileExtension}`
    );
    const date = new Date();
  ...
  it('getData - invalid body', async () => {
    await fs.promises.writeFile(
      `backup\\test-2${BackupService.fileExtension}`,
      JSON.stringify({})
    );
    const fmp: FMPService = new FMPService(
      undefined,
  ...
  it('saveDate - no path', async () => {
    const fmp: FMPService = new FMPService(
      undefined,
      ConfigurationUtil.FmpApiKey()
    );
    expect(await fmp.getData('https://fintual.cl/api/banks')).not.toEqual({});
  });
  it('saveDate - no path', async () => {
    const fmp: FMPService = new FMPService(
      undefined,
    ...
    expect(await fmp.saveDate('https://fintual.cl/api/banks')).not.toEqual({});
  });
});
